...
protected:
    ...
    explicit CompileTimePacketBase(uint8_t* buffer) noexcept : buffer_(buffer), prologue_(buffer) {}

public:
    // Prevent copying (packet is a view over buffer)
    CompileTimePacketBase(const CompileTimePacketBase&) = delete;
    CompileTimePacketBase& operator=(const CompileTimePacketBase&) = delete;

    // Allow moving
    CompileTimePacketBase(CompileTimePacketBase&&) noexcept = default;
    CompileTimePacketBase& operator=(CompileTimePacketBase&&) noexcept = default;
